...
async function markPosted(url, titre, article = {}) {
  if (!url) return;
  const db = await getDb();
  await db.query(
    `INSERT INTO posted_articles (url, titre, resume, source, format, format_emoji, format_label, date_article)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT (url) DO NOTHING`,
    [url, titre, article.resume||'', article.source||'', article.format||'', article.formatEmoji||'📰', article.formatLabel||'ACTUALITÉ', article.date||'']
  );
  await db.end();
}
...
{"articles":[{"titre":"...","resume":"max 150 chars","source":"...","handle":"","url":"https://...","langue":"fr or rn","date":"YYYY-MM-DD","format":"${format.type}"}]}`;
  ...
  // Smart fallback: if a format returns 0 results, try next format
  async function searchWithFallback(group) {
    let results = await searchGroup(group);
    if (results.length === 0) {
      const fallback = getNextFormat();
      addLog(`${group.label} vide → essai ${fallback.label}`, 'info');
      await new Promise(r => setTimeout(r, 3000));
      const fallbackGroup = { label: fallback.label, format: fallback, prompt: buildPrompt(fallback) };
      results = await searchGroup(fallbackGroup);
      addLog(`Fallback ${fallback.label}: ${results.length} résultats`, results.length > 0 ? 'ok' : '');
    }
    return results;
  }

  const g1 = await searchWithFallback(GROUPS[0]);
  addLog(`${format1.label}: ${g1.length} résultats`, g1.length > 0 ? 'ok' : '');
  await new Promise(r => setTimeout(r, 6000));
  const g2 = await searchWithFallback(GROUPS[1]);
  addLog(`${format2.label}: ${g2.length} résultats`, g2.length > 0 ? 'ok' : '');
  await new Promise(r => setTimeout(r, 6000));
  const g3 = await searchWithFallback(GROUPS[2]);
