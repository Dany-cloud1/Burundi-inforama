...
function isEnglish(text) {
  ...
  // Common English words that wouldn't appear in French/Kirundi
...
// Extract date from URL if present (e.g. /2026/04/23/)
...
function isWithin3Days(dateStr, url) {
  ...
  // Fall back to provided date string
...
async function fetchNews() {
  ...
  // Two source groups — searched separately to get more variety
  const GROUPS = [
    {
    ...
    },
    {
      ...
      sources: 'Rugurika on X, RTNBurundi on X, kwaNtare on X, nshingamateka on X, Pacifique Nininahazwe Facebook (facebook.com/pacininahazwe), pnininahazwe on X, ndondeza.org, Baratuza2000 on X, RT_Isanganiro on X'
    }
  ];
...
Search ONLY these sources: ${group.sources}
Return between 1 and 5 recent articles from DIFFERENT sources.

Rules:
- ONLY articles published on or after ${minus3} — strictly NO older articles
- ONLY articles with a real URL starting with http
- ONLY French or Kirundi — NO English
- The "date" field MUST be filled with the real publication date in YYYY-MM-DD format
- If you cannot find the exact date, DO NOT include that article
- Do NOT return an error — always return JSON even with 1 article
- Kirundi articles are welcome${avoidBlock}
...
{"articles":[{"titre":"...","resume":"max 100 chars in original language","source":"...","handle":"@...","url":"https://...","langue":"fr or rn","categorie":"politique/economie/societe/droits/sport","date":"YYYY-MM-DD"}]}`;
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 2000,
          system: 'You are a Burundi news collector. Use web_search. ONLY French or Kirundi articles, NEVER English. Respond ONLY with valid JSON.',
  ...
  addLog('Recherche multi-sources (2 groupes)...', 'info');

  // Search group 1
  addLog('Recherche ' + GROUPS[0].label + '...', 'info');
  const group1Articles = await searchGroup(GROUPS[0]);
  addLog(`${GROUPS[0].label}: ${group1Articles.length} articles`, group1Articles.length > 0 ? 'ok' : '');

  // Wait 6 seconds between calls to avoid rate limit
  await new Promise(r => setTimeout(r, 6000));

  // Search group 2
  addLog('Recherche ' + GROUPS[1].label + '...', 'info');
  const group2Articles = await searchGroup(GROUPS[1]);
  addLog(`${GROUPS[1].label}: ${group2Articles.length} articles`, group2Articles.length > 0 ? 'ok' : '');

  const allArticles = [...group1Articles, ...group2Articles];
  addLog(`Total: ${allArticles.length} articles recus`, allArticles.length > 0 ? 'ok' : '');
  const articles = allArticles;

  // --- FILTER: remove old, English, no-URL, or already-posted articles ---
  const fresh = [];
